...
static int drop_rt_vdso(struct vm_area_list *vma_area_list, struct vdso_quarter *addr, struct vma_area *rt_vdso_marked)
{
	struct vma_area *proxy_vdso = NULL;
	struct vma_area *proxy_vvar = NULL;
	...
	/*
	 * Find the proxy vdso/vvar, since they're unknown to the kernel.
	...
	/*
	 * A guard on either proxy mapping prevents us from collapsing the
	 * proxy/runtime pair. Keep the proxy mappings anonymous, so that the
	 * normal page-dump path skips their guard pages, and retain the marked
	 * runtime pair as the vDSO/VVAR pair in the image.
	 */
	if ((proxy_vdso && proxy_vdso->guarded.any) || (proxy_vvar && proxy_vvar->guarded.any)) {
		if (addr->rt_vvar != VVAR_BAD_ADDR && !rt_vvar_marked) {
			pr_err("Can't find VVAR for marked vDSO retained by guarded proxy\n");
			return -1;
		}

		if (proxy_vdso) {
			proxy_vdso->e->status |= VMA_AREA_REGULAR;
			proxy_vdso->e->status &= ~VMA_AREA_VDSO;
		}
		if (proxy_vvar) {
			proxy_vvar->e->status |= VMA_AREA_REGULAR;
			proxy_vvar->e->status &= ~VMA_AREA_VVAR;
		}

		rt_vdso_marked->e->status |= VMA_AREA_REGULAR | VMA_AREA_VDSO;
		if (rt_vvar_marked)
			rt_vvar_marked->e->status |= VMA_AREA_REGULAR | VMA_AREA_VVAR;

		pr_debug("vdso: Keep marked runtime pair for guarded proxy\n");
		return 0;
	}

	if (proxy_vdso) {
		proxy_vdso->e->status |= VMA_AREA_REGULAR | VMA_AREA_VDSO;
		pr_debug("vdso: Restore orig vDSO status at %lx\n", (long)proxy_vdso->e->start);
	}
	if (proxy_vvar) {
		proxy_vvar->e->status |= VMA_AREA_REGULAR | VMA_AREA_VVAR;
		pr_debug("vdso: Restore orig VVAR status at %lx\n", (long)proxy_vvar->e->start);
	}
...
int parasite_fixup_vdso(struct parasite_ctl *ctl, pid_t pid, struct vm_area_list *vma_area_list)
{
	...
	if (drop_rt_vdso(vma_area_list, &addr, rt_vdso_marked))
		goto out;
	ret = 0;
